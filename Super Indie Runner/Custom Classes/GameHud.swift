...
class GameHud: SKSpriteNode, HudDelegate {
    ...
    init(width size: CGSize) {
        ...
        superCoinCounter.position = CGPoint(x: frame.minX + superCoinCounter.frame.size.width / 1.5, y: frame.midY)
        ...
        for i in 0...2 {
            ...
            emptySlot.position = CGPoint(
                    x: -superCoinCounter.size.width / 2 + CGFloat(i) * superCoinCounter.size.width / 3 + superCoinCounter.size.width * 0.05,
